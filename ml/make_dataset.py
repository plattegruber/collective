...
Usage:
  python make_dataset.py \
      --assets-dir data/raw/assets \
      --out artifacts/datasets \
      --img-size 640 \
      --samples-per-piece 200 \
      --backgrounds-dir data/raw/backgrounds

Dependencies:
  pip install pillow opencv-python numpy albumentations tqdm

Notes:
- We infer class IDs from folder names under data/raw/assets/2d/* and .../3d/*.
...
def main():
    ...
    ap.add_argument('--assets-dir', type=Path, default=Path('data/raw/assets'))
    ap.add_argument('--out', type=Path, default=Path('artifacts/datasets'))
    ap.add_argument('--backgrounds-dir', type=Path, default=Path('data/raw/backgrounds'))
