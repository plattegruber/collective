...
Two output modes:
  1) TFJS:   Uses onnx-tf -> SavedModel -> tensorflowjs_converter -> models/detector/
  2) ONNXWEB: Copies ONNX to models/detector/model.onnx for onnxruntime-web.
...
def main():
    ...
    ap.add_argument('--out', type=Path, required=True, help='Root models directory (e.g., models)')
    ap.add_argument('--public', type=Path, default=Path('public'), help='Public web root for manifest')
